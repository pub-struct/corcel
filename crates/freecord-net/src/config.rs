...
//! connection in freecord-net is built with (PROJECT.md decisions 4 and 5 —
...
/// The RTP payload type freecord uses for H264. Fixed (rather than
/// negotiated) because both sides of every connection are built from this
/// same `media_engine()`.
pub const H264_PAYLOAD_TYPE: u8 = 102;

/// The RTP payload type freecord uses for Opus, for the same reason as
/// [`H264_PAYLOAD_TYPE`].
pub const OPUS_PAYLOAD_TYPE: u8 = 111;

/// The Opus parameters every freecord audio track (published or forwarded)
...
/// The H264 parameters every freecord video track is built with — see
/// [`opus_codec`].
...
/// Builds a [`MediaEngine`] that only knows the codecs freecord speaks: Opus
