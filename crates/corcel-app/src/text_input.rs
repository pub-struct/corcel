//! A minimal single-line text input, adapted from gpui's own `input.rs`
//! example (gpui ships no built-in text input widget). Used for the server
//! name field and the paste-an-invite-link field.
...
/// Registers the key bindings [`TextInput`] reacts to. Call once at startup.
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let (display_text, text_color) = if content.is_empty() {
            (input.placeholder.clone(), theme::faint_foreground().into())
        } else {
            (content, style.color)
        };
