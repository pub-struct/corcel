...
//! own local `corcel-signal` relay as [`ClientMessage::Host`], and routes
...
use corcel_signal::client::Connection;
...
impl HostRelay {
    ...
    /// connection to its local `corcel-signal` relay, opened with
            ...
            match msg {
                ServerMessage::Relay { from, payload } => {
                    let endpoint = match relay.endpoint_for(from).await {
                        ...
                        Err(err) => {
                            eprintln!("corcel-net: failed to set up endpoint for {from}: {err:#}");
                            continue;
                        }
                    };
                    ...
                    if let Err(err) = signal::handle(&endpoint.pc, &relay.outbound, from, payload, false).await {
                        eprintln!("corcel-net: signaling error for {from}: {err:#}");
                    }
                }
    ...
    async fn add_forwarded_track(&self, target: &Arc<Endpoint>, publisher: PeerId, published: PublishedTrack) {
        ...
        let local_track = Arc::new(TrackLocalStaticRTP::new(MediaStreamTrack::new(
            format!("corcel-{publisher}"),
            format!("corcel-{publisher}-{:?}-{ssrc}", published.kind),
            format!("corcel-{publisher}-{:?}", published.kind),
            published.kind,
        ...
        {
            eprintln!(
                "corcel-net: failed to add forwarded track from {publisher} to {}: {err:#}",
                target.peer
            );
            return;
        }
...
impl RelayHandler {
    async fn renegotiate(&self) -> anyhow::Result<()> {
        ...
        self.relay.outbound.send(ClientMessage::Relay {
            to: self.peer,
            payload: corcel_signal::SignalPayload::Offer { sdp: offer.sdp },
        })?;
        Ok(())
    }
}
...
impl PeerConnectionEventHandler for RelayHandler {
    async fn on_ice_candidate(&self, event: RTCPeerConnectionIceEvent) {
        ...
        let _ = self.relay.outbound.send(ClientMessage::Relay {
            to: self.peer,
            payload: corcel_signal::SignalPayload::IceCandidate {
    ...
    async fn on_negotiation_needed(&self) {
        ...
        if let Err(err) = result {
            eprintln!("corcel-net: renegotiation with {} failed: {err:#}", self.peer);
        }
    }
