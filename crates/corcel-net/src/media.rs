//! Bridges `corcel-media`'s RTP packet streams to a [`crate::Participant`]'s
...
/// `corcel-app` hold onto and pass around "the connection to publish more
...
impl OutgoingTrack {
    /// Writes one RTP packet (e.g. from `corcel_media::capture`'s output).
