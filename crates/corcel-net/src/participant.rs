...
use corcel_signal::{ClientMessage, PeerId, ServerMessage, SignalPayload};
use tokio::sync::mpsc;
...
pub async fn join(mut conn: Connection) -> anyhow::Result<Participant> {
    ...
    peer_connection
        .create_data_channel("corcel-init", None)
        .await?;
            ...
            match msg {
                ServerMessage::Relay { from, payload } if from == host => {
                    ...
                    if let Err(err) = signal::handle(&driven_pc, &outbound, from, payload, true).await {
                        eprintln!("corcel-net: signaling error with host: {err:#}");
                    }
                }
                ServerMessage::Relay { .. }
                ...
                ServerMessage::Error { message } => {
                    eprintln!("corcel-net: signaling error: {message}");
                }
            }
        }
    });
...
impl PeerConnectionEventHandler for ParticipantHandler {
    ...
    async fn on_negotiation_needed(&self) {
        ...
        if let Err(err) = result {
            eprintln!("corcel-net: renegotiation with host failed: {err:#}");
        }
    }

    async fn on_connection_state_change(&self, state: RTCPeerConnectionState) {
        if matches!(state, RTCPeerConnectionState::Failed | RTCPeerConnectionState::Closed) {
            eprintln!("corcel-net: connection to host {} is {state:?}", self.host);
        }
    }
