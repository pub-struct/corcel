...
//! (which local track to write them to, at what SSRC) is `freecord-net`'s
...
impl Drop for Capture {
    fn drop(&mut self) {
        ...
        // `freecord-app::runtime::spawn_and_send`), so a runtime handle is
    ...
    std::thread::spawn(move || {
        while let Ok(sample) = sink.pull_sample() {
            ...
            match rtp::unmarshal(&map) {
                ...
                Err(err) => {
                    eprintln!("freecord-media: {label}: failed to parse RTP packet: {err:#}")
                }
            }
        }
    });
...
pub async fn screen() -> anyhow::Result<Capture> {
    ...
    let on_close: Pin<Box<dyn Future<Output = ()> + Send>> = Box::pin(async move {
        if let Err(err) = session.close().await {
            eprintln!("freecord-media: screen: failed to close ScreenCast portal session: {err:#}");
        }
    });
