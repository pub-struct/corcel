//! Glue between the UI and `freecord-signal`/`freecord-net`: creating a
...
use freecord_net::{CallHandle, Participant};
use freecord_signal::{ChannelId, ClientMessage, RelayIdentity};
...
async fn spawn_and_host(
    ...
    let relay =
        freecord_signal::relay::spawn(SocketAddr::new(IpAddr::V4(Ipv4Addr::UNSPECIFIED), port), identity).await?;
    ...
    for channel in &channels {
        ...
        tokio::spawn(async move {
            let conn = match freecord_signal::client::connect(loopback, fingerprint, ClientMessage::Host { channel: id }).await {
                Ok(conn) => conn,
                Err(err) => {
                    eprintln!("freecord-app: failed to host channel {id}: {err:#}");
                    return;
                }
            };
            if let Err(err) = freecord_net::HostRelay::run(conn).await {
                eprintln!("freecord-app: host relay for channel {id} exited: {err:#}");
            }
        });
    }
...
pub async fn open_room(
    ...
    fingerprint: [u8; 32],
    server_id: Uuid,
) -> anyhow::Result<freecord_signal::client::Connection> {
    freecord_signal::client::connect(addr, fingerprint, ClientMessage::Room { channel: server_id }).await
}
...
/// [`freecord_signal::client`]'s doc comment on why the host also connects
...
async fn join_at(addr: SocketAddr, fingerprint: [u8; 32], channel: ChannelId) -> anyhow::Result<CallSession> {
    let conn = freecord_signal::client::connect(addr, fingerprint, ClientMessage::Join { channel }).await?;
    let participant = freecord_net::join(conn).await?;
...
pub struct CallSession {
    pub pc: CallHandle,
    pub remote_video: mpsc::Receiver<freecord_media::VideoFrame>,
    ...
    pub local_video: mpsc::Sender<freecord_media::VideoFrame>,
}
...
async fn attach_media(participant: Participant) -> anyhow::Result<CallSession> {
    ...
    let mut mic = freecord_media::capture::microphone()?;
    let outgoing = freecord_net::publish_audio_track(&pc).await?;
    ...
    // Same bound/drop-when-full reasoning as freecord_media::playback's own
    ...
    let (video_tx, video_rx) = mpsc::channel(freecord_media::playback::FRAME_CHANNEL_CAPACITY);
    ...
    tokio::spawn(async move {
        loop {
            ...
            if let Some(mut packets) = freecord_net::subscribe_audio(track.clone()).await {
                let playback = match freecord_media::AudioPlayback::new() {
                    Ok(playback) => playback,
                    Err(err) => {
                        eprintln!("freecord-app: failed to start audio playback: {err:#}");
                        continue;
                    }
                };
                let mut stop_rx = hang_up_rx.clone();
            ...
            if let Some(mut packets) = freecord_net::subscribe_video(track).await {
                let mut playback = match freecord_media::VideoPlayback::new() {
                    Ok(playback) => playback,
                    Err(err) => {
                        eprintln!("freecord-app: failed to start video playback: {err:#}");
                        continue;
                    }
                };
...
pub async fn start_camera(
    pc: CallHandle,
    device: &str,
    preview: mpsc::Sender<freecord_media::VideoFrame>,
) -> anyhow::Result<CameraHandle> {
    let mut capture = freecord_media::capture::camera(device)?;
    let outgoing = freecord_net::publish_video_track(&pc).await?;
    let mut playback = freecord_media::VideoPlayback::new()?;
...
/// the same [`freecord_media::VideoPlayback`] path remote video uses rather
...
pub async fn start_screen_share(
    ...
    let mut capture = freecord_media::capture::screen().await?;
    let outgoing = freecord_net::publish_video_track(&pc).await?;
    let mut playback = freecord_media::VideoPlayback::new()?;
