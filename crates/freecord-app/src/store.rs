...
use freecord_signal::{ChannelId, RelayIdentity};
...
/// `$XDG_CONFIG_HOME/freecord/freecord.db` — deliberately the same directory
/// as `profile.json` (see [`crate::profile`]) so "everything freecord knows"
/// lives in one place the user can find, back up, or delete.
fn db_path() -> PathBuf {
    let config_home = std::env::var_os("XDG_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".config")))
        .unwrap_or_else(|| PathBuf::from("."));
    config_home.join("freecord").join("freecord.db")
}
