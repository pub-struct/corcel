...
use freecord_signal::{ClientMessage, ServerMessage, SignalPayload};

#[tokio::test]
async fn host_and_participant_exchange_a_signal() {
    let bind = SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 0);
    let relay = freecord_signal::relay::spawn(bind)
        .await
    ...
    let mut host = freecord_signal::client::connect(
        addr,
        relay.fingerprint,
    ...
    let mut participant = freecord_signal::client::connect(
        addr,
        relay.fingerprint,
